...
OLSRForward::configure(Vector<String> &conf, ErrorHandler *errh)
{
    ...
    if (cp_va_kparse(conf, this, errh,
                     "DUPLICATE_HOLDING_TIME", cpkP+cpkM, cpInteger, &dup_hold_time,
                     "DUPLICATESET_ELEMENT", cpkP+cpkM, cpElement, &_duplicateSet,
                     ...
                     "MAIN_IP_ADDRESS", cpkP+cpkM, cpIPAddress, &_myMainIP,
                     cpEnd) < 0)
        return -1;
    _dup_hold_time=Timestamp::make_msec (dup_hold_time,0);
    return 0;
}
