...
OLSRNeighborInfoBase::configure(Vector<String> &conf, ErrorHandler *errh)
{
	...
	if ( cp_va_kparse(conf, this, errh,
	                 ...
	                 "HELLO_GENERATOR_ELEMENT", cpkP+cpkM, cpElement, &_helloGenerator,
	                 "LINK_INOFRMATION_BASE", cpkP+cpkM, cpElement, &_linkInfoBase,
	                 "INTERFACE_INFORMATION_BASE", cpkP+cpkM, cpElement, &_interfaceInfoBase,
	                 "NODES_MAIN_IP_ADDRESS", cpkP+cpkM, cpIPAddress, &_myMainIP,
	                 "ADDITIONAL_HELLO", cpkN, cpBool, &add_hello_msg,
	                 "ADDITIONAL_MPRS", cpkN, cpBool, &add_mprs,
...
OLSRNeighborInfoBase::mpr_selector_expiry_hook(Timer *timer, void *thunk)
{
	...
	Timestamp now, next_timeout;
    now = Timestamp::now();
	next_timeout = Timestamp::make_msec(0, 0);
...
OLSRNeighborInfoBase::twohop_expiry_hook(Timer *timer, void *thunk)
{
	...
	if (twohop_removed)
	{
		nib->_routingTable->compute_routing_table(false,now);
        //if (_fullTopologyTable != 0){
            //nib->_fullTopologyTable->compute_routing_table(true, now);
            //nib->_fullTopologyTable->print_routing_table(true);
        //}
		nib->compute_mprset();
	}
}
...
OLSRNeighborInfoBase::add_neighbor(IPAddress neigh_addr)
{
	...
	data->link_quality = OLSR_METRIC_INFINITY;
	data->N_link_quality = OLSR_METRIC_INFINITY;
    data->link_variance = 0;
    data->N_link_variance = 0;
...
bool
OLSRNeighborInfoBase::update_neighbor(IPAddress neigh_addr, int status, int willingness, int link_quality, int N_link_quality)
{
	struct neighbor_data *data;
	...
	if ( data != 0 )
	{
		...
		//COREY - Added to include ETX
		data->link_quality = link_quality;
		data->N_link_quality = N_link_quality;
		return true;
	}
	return false;
}
...
OLSRNeighborInfoBase::print_neighbor_set()
{
	if (! _neighborSet->empty())
	{
		for (NeighborSet::iterator iter = _neighborSet->begin(); iter != _neighborSet->end(); iter++)
		{
			...
			click_chatter("\tlink_quality: %d\n", data->link_quality );
            click_chatter("\tN_link_quality: %d\n", data->N_link_quality );
            click_chatter("\tlink_variance: %d\n", data->link_variance );
			click_chatter("\tN_link_variance: %d\n", data->N_link_variance );
            click_chatter("\tETX: %d\n", data->etx );
            click_chatter("\tVariance: %d\n", data->variance);
...
bool
OLSRNeighborInfoBase::add_twohop_neighbor(IPAddress neigh_addr, IPAddress twohop_neigh_addr, Timestamp time, struct neighbor_data *N)
{
	...
	data->N_time = time;
    data->N_status = N->N_status;
    data->N_willingness = N->N_willingness;
...
#else
    /*neighbor_data *neigh = find_neighbor (neigh_addr);
    
    if (neigh != 0){
        data->etx = neigh->etx + N->etx;
        data->variance = neigh->variance + N->variance;
        //click_chatter("*****Neigh etx %d and N_etx %d", neigh->variance, N->variance);
        data->N_etx = N->etx;
        data->N_variance = N->variance;
        data->etx_time = time;
        data->var_time = time;
        data->etx_min = data->etx;
        data->etx_min_time = time;
        data->etx_max = data->etx;
        data->etx_max_time = time;
    }else{
        //click_chatter("*********ERROR: This should never be able to happen********");*/
        data->etx = OLSR_METRIC_INFINITY;
        data->variance = OLSR_METRIC_INFINITY;
        data->N_etx = OLSR_METRIC_INFINITY;
        data->N_variance = 0;
        data->etx_time = Timestamp::make_msec(0,0);
        data->var_time = Timestamp::make_msec(0,0);
        data->etx_min = OLSR_METRIC_INFINITY;
        ...
        data->N_etx_time = Timestamp::make_msec(0,0);
        data->N_var_time = Timestamp::make_msec(0,0);
        data->N_etx_min = OLSR_METRIC_INFINITY;
        data->N_etx_min_time = Timestamp::make_msec(0,0);
        data->N_etx_max = OLSR_METRIC_INFINITY;
        data->N_etx_max_time = Timestamp::make_msec(0,0);
        data->N_etx_delta_time = Timestamp::make_msec(0,0);
    //}
	...
	if (data2 != 0)
	{
        ...
        if (data2->etx < OLSR_METRIC_INFINITY){
            data->etx = data2->etx;
            data->etx_time = data2->etx_time;
            data->variance = data2->variance;
            data->var_time = data2->var_time;
            
            if ((data2->etx_min < data->etx_min) || (data->etx_min == 0)){
                data->etx_min = data2->etx_min;
                data->etx_min_time = data2->etx_min_time;
            }
            
            if ((data2->etx_max > data->etx_max) || (data->etx_max == OLSR_METRIC_INFINITY)){
                data->etx_max = data2->etx_max;
                data->etx_max_time = data2->etx_max_time;
            }
        }
        
        //Compare previous neighbor info with new neighbor info only if it was real
        if (data2->N_etx < OLSR_METRIC_INFINITY){
            data->N_etx = data2->N_etx;
            data->N_etx_time = data2->N_etx_time;
            data->N_variance = data2->N_variance;
            data->N_var_time = data2->N_var_time;
            
            if ((data2->N_etx_min < data->N_etx_min) || (data->N_etx_min == 0)){
                data->N_etx_min = data2->N_etx_min;
                data->N_etx_min_time = data2->N_etx_min_time;
            }
            
            if ((data2->N_etx_max > data->N_etx_max) || (data->N_etx_max == 0)){
                data->N_etx_max = data2->N_etx_max;
                data->N_etx_max_time = data2->N_etx_max_time;
            }
        }
...
OLSRNeighborInfoBase::print_twohop_set()
{
	if (! _twohopSet->empty() )
	{
		for (TwoHopSet::iterator iter = _twohopSet->begin(); iter != _twohopSet->end(); iter++)
		{
			...
			click_chatter("\tN_time: %f\n", Timestamp( data->N_time ).doubleval());
            click_chatter("\tETX: %d\n", data->etx );
            click_chatter("\tVariance: %d\n", data->variance);
            click_chatter("\tN_etx: %d\n", data->N_etx );
            click_chatter("\tN_variance: %d\n", data->N_variance );
		}
	}
	else
	{
...
mpr_selector_data *
OLSRNeighborInfoBase::add_mpr_selector(IPAddress ms_addr, Timestamp time)
{
