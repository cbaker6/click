...
public:
    ...
    bool update_neighbor(IPAddress neigh_addr, int status, int willingness, int link_quality=-1, int N_link_quality=-1);
    void remove_neighbor(IPAddress neigh_addr);
    void print_neighbor_set();
    HashMap<IPAddress, void *> *get_neighbor_set();
    
    bool add_twohop_neighbor(IPAddress neigh_addr, IPAddress twohop_neigh_addr, Timestamp time, struct neighbor_data*/*int N_etx=-1, int N_variance=-1, int N_status=-1, int N_willingness=-1*/  );
    ...
    struct mpr_selector_data *add_mpr_selector(IPAddress ms_addr, Timestamp time);
