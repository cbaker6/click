...
OLSRAssociationInfoBase::configure(Vector<String> &conf, ErrorHandler *errh)
{
  ...
  if ( cp_va_kparse( conf, this, errh,
		    ...
		    "USE_TIMER", cpkN, cpBool, &_useTimer,
			"REDUNDANCY_CHECK", cpkN, cpBool, &_redundancyCheck,
			"HNA_COMPACTING", cpkN, cpBool, &_compact,
			"HOME_NETWORK", cpkN, cpIPAddress, &_home_network,
			"HOME_NETWORK_NETMASK", cpkN, cpIPAddress, &_home_netmask,
...
association_data *
OLSRAssociationInfoBase::add_tuple(IPAddress gateway_addr, IPAddress network_addr, IPAddress netmask, Timestamp time)
{
...
OLSRAssociationInfoBase::set_home_network_write_handler(const String &conf, Element *e, void *, ErrorHandler * errh)
{
  ...
  int res = cp_va_kparse(conf, me, errh,
			"NETWORK_ADDRESS_HNA_SHOULD_ADVERTIS", cpkP + cpkM, cpIPPrefix, &me->_home_network, &me->_home_netmask,
			cpEnd);
  if ( res < 0 )
    return res;  
  return 0;
