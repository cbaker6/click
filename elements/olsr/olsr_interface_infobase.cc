...
OLSRInterfaceInfoBase::configure(Vector<String> &conf, ErrorHandler *errh)
{
    ...
    if ( cp_va_kparse(conf, this, errh,
                      ...
                      "LOCAL_INTERFACES_INFORMATION_BASE", cpkP+cpkM, cpElement, &_localIfInfoBase,
...
bool
OLSRInterfaceInfoBase::add_interface(IPAddress iface_addr, IPAddress main_addr, Timestamp time)
{
...
bool
OLSRInterfaceInfoBase::update_interface(IPAddress iface_addr, Timestamp time){
    interface_data *data;
    data = find_interface(iface_addr);
    if ( ! data == 0 ) {
        data->I_time = time;
